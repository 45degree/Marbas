...
其中`Buffer`有`UBO`和`Dynamic UBO`两种, 而`Image`主要是指`Texture`, `TextureCubeMap`

> TODO: 可能需要`SSBO`, 不过目前还用不到

每一个`Descriptor`都需要指定一个绑定点

#### DescriptorSetLayoutBinding and DescriptorSetLayout

`DescriptorSetLayoutBinding` 用来描述绑定点信息
...
多个`DescriptorSetLayoutBinding`可以用来创建一个`DescriptorSetLayout`,
`DescriptorSetLayout`是用来创建`DescriptorSet`的工具

#### Descriptor Set 

将多个`Descriptor`形成一个`DescriptorSet`,

#### Descriptor Set

每条`pipeline` 中可以有多个`DescriptorSet`, 这些`Descriptor`被称为`Descriptor Set`。
