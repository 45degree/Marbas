...
void Config::Load() { LoadLocaleFile(); }
...
void Config::LoadLocaleFile() {
  try {
    ...
    if (!menuList.is_array()) {
      String errorMsg = FORMAT("can't parse item {} in locale config file: {}", "menu",
                               m_localeFilePath.string());
    ...
    for (auto& item : *menuItems) {
      if (!item.is_string()) {
        String errorMsg =
            FORMAT("can't parse item {} in locale config file: {}", "menu",
                   m_localeFilePath.string()) throw std::runtime_error(errorMsg.c_str());
      }
