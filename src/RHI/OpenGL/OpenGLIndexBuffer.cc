...
OpenGLIndexBuffer::OpenGLIndexBuffer(size_t count) : IndexBuffer(count) {
...
OpenGLIndexBuffer::OpenGLIndexBuffer(const Vector<uint32_t>& data) : IndexBuffer(data.size()) {
  LOG(INFO) << "create opengl index buffer";
...
OpenGLIndexBuffer::SetData(const Vector<uint32_t>& data, size_t offsetCount) {
  LOG(INFO) << FORMAT(
      ...
      " offset count is {}",
      m_indexCount, offsetCount);

  if (offsetCount + data.size() > m_indexCount) {
    LOG(ERROR) << FORMAT("can't set data for buffer, this buffer's max count is {}", m_indexCount);
    return;
  }
