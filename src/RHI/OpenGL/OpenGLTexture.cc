...
void OpenGLTextureCubeMap::SetData(void* data, uint32_t size, CubeMapPosition position) {
    ...
    int layer = 0;
    switch(position) {
    case CubeMapPosition::BACK: layer = 4; break;  // GL_TEXTURE_CUBE_MAP_POSITIVE_Z;
    case CubeMapPosition::FRONT: layer = 5; break; // GL_TEXTURE_CUBE_MAP_NEGATIVE_Z
    case CubeMapPosition::TOP: layer = 2; break;
    case CubeMapPosition::BOTTOM: layer = 3; break;
    case CubeMapPosition::LEFT: layer = 0; break;
    case CubeMapPosition::RIGHT: layer = 1; break;
    }

    glTextureSubImage3D(m_textureID, 1, 0, 0, layer, m_width, m_height, 1,
...
void OpenGLTextureCubeMap::Bind(int bindingPoint) {
    ...
    auto error = glGetError();
    LOG_IF(ERROR, error) << FORMAT("can't bind texture {} to {}, error code is {}",
        m_textureID, bindingPoint, error);
}
...
void OpenGLTextureCubeMap::UnBind() {
    ...
    auto error = glGetError();
    LOG_IF(ERROR, error) << FORMAT("can't unbind texture: {}, error code is {}",
        m_textureID, error);
}
