...
void OpenGLVertexArray::EnableVertexAttribArray(const VertexBuffer* vertexBuffer) const {
  ...
  for (const auto& elementInfo : layout) {
    ...
    switch (elementInfo.mateType) {
      ...
      case ElementType::INT:
      case ElementType::UNSIGNED_INT:
        glVertexAttribIPointer(index, size, ConvertToOpenGLType(elementInfo.mateType), stride,
                               offset);
        break;
      case ElementType::FLOAT:
      case ElementType::DOUBLE:
      case ElementType::HALF_FLOAT:
        glVertexAttribPointer(index, size, ConvertToOpenGLType(elementInfo.mateType), isNormalized,
                              stride, offset);
    }
  }
}
