...
class RenderLayer : public LayerBase {
 ...
 public:
  ...
  [[nodiscard]] const Viewport* GetViewport() const noexcept { return m_viewport.get(); }

  Camera* GetCamera() { return m_editorCamera.get(); }

  void SetSecne(std::unique_ptr<Scene>&& scene) { m_scene = std::move(scene); }

  [[nodiscard]] Scene* GetScene() const { return m_scene.get(); }
